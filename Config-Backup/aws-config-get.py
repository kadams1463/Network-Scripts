...
import boto3 # used for accessing AWS Secrets Manager
from boto3 import Session # additional library to set the session for AWS
...
# AWS SECRETS MANAGER
# Set the session type for AWS
client = boto3.client('secretsmanager', region_name='us-east-1')

# Get the password for the user account
response1 = client.get_secret_value(SecretId='prod/project-name/password-or-secrett')

# Set the variable for that password
user_password = json.loads(response1['SecretString'])

# Get the enable password for the user account
response2 = client.get_secret_value(SecretId='prod/project-name/password-or-secret')

# Set the variable on the enable password
user_secret = json.loads(response2['SecretString'])
